...
std::vector<FileInfo> StatisticsCounter::CountProjectStats(std::map<std::string, std::string> files)
{
	...
	{
		futures.push_back(std::async(std::launch::async, &StatisticsCounter::CountFileStats, this, current_file));
...
FileInfo StatisticsCounter::CountFileStats(std::pair<std::string, std::string> file)
{
	...
	blank_lines_and_comments.merge(comment_intervals);
...
std::map<unsigned, unsigned> StatisticsCounter::FindCommentSections(std::string file_text) {
	...
	{
		found_pos_end = file_text.find("*/", (found_pos_start == 0) ? 0 : found_pos_start + 1);
	...
	std::map<unsigned, unsigned>& comment_intervals, std::map<unsigned, unsigned>& lines_without_code)
{
	...
	while (it != end)
	{
		...
		if ((file_text.length() > pos2 && file_text[pos2] == '\n') || pos2 == file_text.length())
		{
			...
			{
				lines_without_code.emplace(pos1 + 1, pos2 + 1);
				++blank_lines_count;
			}
		}
		++it;
...
unsigned StatisticsCounter::CountCodeLines(std::string file_text, std::map<unsigned, unsigned>& comment_and_blank_intervals)
{
	...
	std::map<unsigned, unsigned>::iterator map_iter{ comment_and_blank_intervals.begin() };
	unsigned prev_element = 0, current_element = map_iter->first;
	map_iter++;
	/*for (auto a : comment_and_blank_intervals)
	{
		std::cout << file_text.substr(a.first, a.second - a.first)<< std::endl;
	}*/
	while (current_element <= file_text.length())
	{
		if (map_iter == comment_and_blank_intervals.end()) break;

		if (prev_element != current_element - 1)
		{
			code_lines_count += std::count(file_text.begin() + prev_element,
				file_text.begin() + current_element, '\n');

			//std::cout << file_text.substr(prev_element, current_element - prev_element);
		}
		if (prev_element == 0)
		{
			auto start_iterator = comment_and_blank_intervals.begin();
			prev_element = start_iterator++->second + 1;
			current_element = start_iterator->first;
		}
		else
		{
			prev_element = map_iter++->second;
			if (map_iter == comment_and_blank_intervals.end())
			{
				if (comment_and_blank_intervals.rbegin()->second != file_text.length())
					current_element = file_text.length() - 1;
				else break;
			}
			else current_element = map_iter->first;
		}
	}
